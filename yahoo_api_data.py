...
class YahooFantasyDataFetcher:
    ...
    """从数据库获取联盟数据，格式化为字典"""
    def _extract_leagues_from_db(self) -> Optional[Dict]:
    ...
    """从API获取用户的games数据"""
    def _pull_YahooAPI_games(self) -> Optional[Dict]:
    ...
    """从games_data中提取game_keys（只包含type='full'的游戏）"""
    ...
    """从API获取指定game下用户的leagues数据"""
    def _pull_YahooAPI_leagues(self, game_key: str) -> Optional[Dict]:
        url = f"https://fantasysports.yahooapis.com/fantasy/v2/users;use_login=1/games;game_keys={game_key}/leagues?format=json"
        return get_api_data(url)
    
    """从API获取league_settings"""
    def _pull_YahooAPI_league_settings(self, league_key: str) -> Optional[Dict]:
    ...
    """从API返回的数据中提取联盟信息"""
    def _extract_leagues_from_YahooAPI(self, data: Dict, game_key: str) -> List[Dict]:
    ...
    def _ensure_league_exists_in_db(self) -> bool:
        ...
        try:
            # 检查联盟是否已存在于数据库中
            from database.model import League, Game
    ...
    def fetch_complete_league_data(self) -> bool:
        ...
        try:
            # 获取联盟设置数据
            settings_data = self._pull_YahooAPI_league_settings(league_key)
            if settings_data:
                # 直接写入数据库
    ...
    def handle_database_error(self) -> bool:
        ...
        try:
            ...
            from database.database_ops import recreate_tables
        ...
        while True:
            ...
            if choice == "0":
                ...
                if self._ensure_league_selected():
                    ...
                    self.fetch_complete_league_data()
    ...
    def select_league(self) -> bool:
        ...
        # 优先从数据库获取联盟数据
        leagues_data = self._extract_leagues_from_db()
        
        if not leagues_data:

            games_data = self._pull_YahooAPI_games()
            if not games_data:
                return None
            ...
            for i, game_key in enumerate(game_keys):
                leagues_data = self._pull_YahooAPI_leagues(game_key)
                if leagues_data:
                    extracted_leagues = self._extract_leagues_from_YahooAPI(leagues_data, game_key)
