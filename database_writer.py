...
class FantasyDatabaseWriter:
    ...
    def __init__(self, batch_size: int = 100):
        ...
        self.engine = DatabaseOps.create_database_engine()
        
        # 检查并修复表结构问题
        if self._check_table_structure_issues():
            print("🔧 检测到数据库表结构问题，正在修复...")
            DatabaseOps.recreate_tables(self.engine)
        else:
            DatabaseOps.create_tables(self.engine)
            print("数据库模型创建完成！")
            print("表包括：")
            for table_name in Base.metadata.tables.keys():
                print(f"  - {table_name}") 
            
        self.session = DatabaseOps.get_session(self.engine)
    ...
    def _check_table_structure_issues(self) -> bool:
        ...
        try:
            # 创建一个临时session来检查表结构
            temp_session = DatabaseOps.get_session(self.engine)
